<?php   
  ...
  {
    $id=$_GET['id'];
    $m = "DELETE FROM category WHERE category_id='$id'";
  ...
  if(count($_POST)>0)
  {
      ...
      switch ($_POST['action'])
      {
        case 'active':
              ...
              {
                 $n = "UPDATE category set status=1 WHERE  category_id='$value'";
        ...
        case 'deactive':
              foreach($_POST['all_id'] as $key => $value)
              {
                $n = "UPDATE category set status=0 WHERE category_id='$value'";
                $n1 = mysqli_query($con,$n);
              }
            break;
         case 'delete':
              foreach($_POST['all_id'] as $key => $value)
              {
                $n = "DELETE FROM category WHERE category_id='$value'";
                $n1 = mysqli_query($con,$n);
              }
            break;
       }
      ...
      <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
          ...
          <?php foreach($data as $key => $value) { ?>
            ...
            <tr>
              <td> <input type="checkbox" name="all_id[]" class="check" value="<?php echo $value['category_id']; ?>"> </td>
              <td><?php echo $value['category_id']; ?></td>
              <td><?php echo $value['category_name']; ?></td>
      

     
              <td class="text-nowrap"><a href="view_category.php?id=<?php echo $value['category_id']; ?>">
                <button class="btn btn-danger" type="button"><i class="fas fa-user-minus"></i> Delete</button>
              </a>

                <a href="edit_category.php?id=<?php echo $value['category_id'] ?>">
