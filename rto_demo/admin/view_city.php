<?php   
  ...
  {
    $id=$_GET['id'];
    $m = "DELETE FROM city WHERE city_id='$id'";
  ...
  $q = "SELECT city.*,state.state_name FROM city,state WHERE state.state_id=city.city_state_id";
...
if(isset($_GET['city_name']))
  {
      ...
      // $ckey = $_GET['city_name'];
      $q = "SELECT city.*,state.state_name FROM state,city WHERE state.state_id=city.city_state_id AND city_name LIKE '%ckey%' AND state_name LIKE '%skey%'";
;
      // die;
  }
  else
  ...
  if(count($_POST)>0)
  {
      ...
      switch ($_POST['action'])
     {
        case 'active':
              ...
              {
                 $n = "UPDATE city set status=1 WHERE  city_id='$value'";
        ...
        case 'deactive':
              foreach($_POST['all_id'] as $key => $value)
              {
                $n = "UPDATE city set status=0 WHERE city_id='$value'";
                $n1 = mysqli_query($con,$n);
              }
            break;
         case 'delete':
              foreach($_POST['all_id'] as $key => $value)
              {
                $n = "DELETE FROM city WHERE city_id='$value'";
                $n1 = mysqli_query($con,$n);
              }
            break;
      }
...
<div class="card">
    <div class="card-header">
      <h3 class="card-title">City DataTable with default features</h3>
    </div>
    <!-- /.card-header -->
    <div class="card-body overflow-auto">
      ...
      <div style="text-align: right;">
        ...
        <a href="add_city.php" class="btn btn-primary">Add</a>
      </div>
      <br>
      ...
      <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
          ...
          <?php foreach($data as $key => $value) { ?>
            ...
            <tr>
              <td> <input type="checkbox" name="all_id[]" class="check" value="<?php echo $value['city_id']; ?>"> </td>
              ...
              <td class="text-nowrap">
                <a href="view_city.php?id=<?php echo $value['city_id']; ?>">
                  <button class="btn btn-danger" type="button"><i class="fas fa-user-minus"></i> Delete</button>
                </a>

                <a href="edit_city.php?id=<?php echo $value['city_id'] ?>">
